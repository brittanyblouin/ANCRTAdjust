...
#'    \itemize{
#'      \item \code{n_status_c_impute} uses the impute adjustment option
#'      \item \code{n_status_c_remove} uses the remove adjustment option
#'      \item \code{n_status_c_setmax} uses the set to maximum adjustment option
#'      }
...
#'        \item \code{totpos_c_impute}: \code{totpos_c} is set to missing when \code{totpos_c} > \code{n_status_c_impute}
#'        \item \code{totpos_c_remove}: \code{totpos_c} is set to missing when \code{totpos_c} > \code{n_status_c_remove}
#'        \item \code{totpos_c_setmax}: \code{totpos_c} is set to missing when \code{totpos_c} > \code{n_status_c_setmax}
#'        }
...
#'   \item \code{n_status_c_impute}:  Adjusted \code{n_status_c} using the impute adjustment option for multiple testing
#'   \item \code{n_status_c_remove}:  Adjusted \code{n_status_c} using the remove adjustment option for multiple testing
#'   \item \code{n_status_c_setmax}:  Adjusted \code{n_status_c} using the set to maximum adjustment option for multiple testing
#'   \item \code{totpos_c}: Cleaned \code{totpos} (according to description)
#'   \item \code{totpos_c_impute}: Adjusted \code{totpos_c} if the impute adjustment option for multiple testing is used
#'   \item \code{totpos_c_remove}: Adjusted \code{totpos_c} if the remove adjustment option for multiple testing is used
#'   \item \code{totpos_c_setmax}: Adjusted \code{totpos_c} if the set to maximum adjustment option for multiple testing is used
#'  }
#'
#' @export
...
data_clean <- function(data, total_age_cat = NULL){
  ...
  if (is.null(data$totpos)) {
    data$totpos <- data$knownpos + data$testpos
  }
  ...
  data$n_status_c_setmax <- ifelse((data$n_status_c > data$n_clients) & !is.na(data$n_status_c) & !is.na(data$n_clients), data$n_clients, data$n_status_c)
  data$n_status_c_remove <- ifelse((data$n_status_c > data$n_clients) & !is.na(data$n_status_c) & !is.na(data$n_clients), NA, data$n_status_c)
  
  mean_cov <- ddply(data, "faciluid", meancov_possible)
  data <- merge(data, mean_cov, by = "faciluid")
  data$facilmeancov <- data$V1
  data$V1 <- NULL
  data$n_status_c_impute <- ifelse(data$n_status_c > data$n_clients & !is.na(data$n_status_c) & !is.na(data$n_clients), round((data$facilmeancov * data$n_clients),0), data$n_status_c)
  
  data$totpos_c <- ifelse(data$n_status_c < data$totposC & !is.na(data$n_status_c) & !is.na(data$totposC), NA, data$totposC)
  data$totpos_c_setmax <- ifelse(data$n_status_c_setmax < data$totposC & !is.na(data$n_status_c_setmax) & !is.na(data$totposC), NA, data$totposC)
  data$totpos_c_impute <- ifelse(data$n_status_c_impute < data$totposC & !is.na(data$n_status_c_impute) & !is.na(data$totposC), NA, data$totposC)
  data$totpos_c_remove <- ifelse(data$n_status_c_remove < data$totposC & !is.na(data$n_status_c_remove) & !is.na(data$totposC), NA, data$totposC)
  
  data$check <- duplicated(data$id_time)
  data$check <- ifelse(data$check == "TRUE", 1, 0)
  
  if (sum(data$check) > 0) { 
    warning("Duplicate observations exist. If age-disaggregated data is included, ensure that 'total_age_cat' has been specified.") }
