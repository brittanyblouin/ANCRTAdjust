...
descriptive_plot <- function (data, ylim.ind_min = 0, ylim.ind_max = 100, ylim.cov_min = 0, ylim.cov_max = 100, 
  ...
  indicators <- ddply(data, "Time", primary_indicators)
  ...
  coverages <- ddply(data, "Time", coverages)
  ...
  coverages$dif_adj <- pmax(coverages$dif_1.2, coverages$dif_1.3, coverages$dif_1.4, coverages$dif_2.3, coverages$dif_2.4, coverages$dif_3.4, na.rm = TRUE)
  ...
  prevalences <- ddply(data, "Time", HIVprevs)
  ...
  prevalences$dif_adj <- pmax(prevalences$dif_1.2, prevalences$dif_1.3, prevalences$dif_1.4, prevalences$dif_2.3, prevalences$dif_2.4, prevalences$dif_3.4, na.rm = TRUE)
  ...
  indicator_plot <-
    ...
    geom_line(aes(y = impdata, color = "Invalid values in raw data"), size=1) + geom_point(aes(y = impdata, color = "Invalid values in raw data"))+
    geom_line(aes(y = missingdata_cleaned, color = "Missing data in cleaned data"), size=1) + geom_point(aes(y = missingdata_cleaned, color = "Missing data in cleaned data"))+
    xlab("") +
  ...
  coverage_plot <-
    ...
    geom_line(aes(y = coverage, color = "Cleaned data"), linetype = "dashed", size=1) + geom_point(aes(y = coverage, color = "Cleaned data"), position=position_jitter(width = 0.02, height = 0))+
    geom_line(aes(y = coverage.impute, color = "Impute option"), linetype = "dashed", size=1) + geom_point(aes(y = coverage.impute, color = "Impute option"))+
    geom_line(aes(y = coverage.setmax, color = "Set to maximum option         "), linetype = "dashed", size=1) + geom_point(aes(y = coverage.setmax, color = "Set to maximum option         "))+
    geom_line(aes(y = coverage.remove, color = "Remove option"), linetype = "dashed", size=1) + geom_point(aes(y = coverage.remove, color = "Remove option"))+
    geom_line(aes(y = coverage_raw, color = "Raw data"), size = 1) + geom_point(aes(y = coverage_raw, color = "Raw data"))+
    ylim(c(ylim.cov_min, ylim.cov_max))+
    xlab("") +
    ylab("HIV testing coverage \n (%)") +
    theme(axis.title.y = element_text(size=9))+
    scale_colour_manual(name="",
                      values=c("Raw data"="red", "Cleaned data"="darkgoldenrod4", "Set to maximum option         "="purple", "Remove option" ="blue", "Impute option" = "forestgreen"))+
    geom_abline (aes(intercept = 100, slope = 0), linetype = "dashed")
    
  
  prevalence_plot <-
    ggplot(prevalences, aes(Time)) +
    geom_line(aes(y = prev, color = "Cleaned data"), linetype = "dashed", size = 1) + geom_point(aes(y = prev, color = "Cleaned data"), position=position_jitter(width = 0.02, height = 0))+
    geom_line(aes(y = prev.impute, color = "Impute option"), linetype = "dashed", size = 1) + geom_point(aes(y = prev.impute, color = "Impute option"))+
    geom_line(aes(y = prev.setmax, color = "Set to maximum option         "), linetype = "dashed", size = 1) + geom_point(aes(y = prev.setmax, color = "Set to maximum option         "))+
    geom_line(aes(y = prev.remove, color = "Remove option"), linetype = "dashed", size = 1) + geom_point(aes(y = prev.remove, color = "Remove option"))+
    geom_line(aes(y = prev_raw, color = "Raw data"), size = 1) + geom_point(aes(y = prev_raw, color = "Raw data"))+
    xlab("Time") +
    ylab("HIV prevalence \n (%)") +
   ...
   if(plot_type=="full"){
     finalplot1 <- ggarrange(indicator_plot, coverage_plot, prevalence_plot, ncol = 1, nrow = 3, legend = "right")
     return(finalplot1)
   }

   if(plot_type=="prev_cov"){
     finalplot2 <- ggarrange(coverage_plot, prevalence_plot, ncol = 1, nrow = 2, legend = "right")
     return(finalplot2)
   }

   if(plot_type=="prev_ind"){
     finalplot3 <- ggarrange(indicator_plot, prevalence_plot, ncol = 1, nrow = 2, legend = "right")
     return(finalplot3)
   }

   if(plot_type=="cov_ind"){
     finalplot4 <- ggarrange(indicator_plot, coverage_plot, ncol = 1, nrow = 2, legend = "right")
     return(finalplot4)
   }
