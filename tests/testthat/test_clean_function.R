...
ancrt_checked <- ANCRTAdjust::check_data(ancrt, faciluid = "faciluid", time = "time", n_clients = 'n_clients', 
...
testthat::test_that("checking data function is working as intended", {
  ...
  expect_true(all(c("faciluid", "time", "n_clients", "n_status", "knownpos",
                    "testpos", "testneg", "snu1", "year") %in% names(ancrt_checked)))
})
